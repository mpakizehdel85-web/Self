...
async def set_scheduled_messages(event):
    ...
    if count <= 0 or interval is None or interval <= 0:
        await event.edit("❌ مقادیر وارد شده نامعتبر است.")
        return
...
# ============================================================
# NEW COMMANDS: .purge, .save, .uptime
# ============================================================
...
async def purge_messages(event):
    ...
    async for message in client.iter_messages(event.chat_id, limit=count, from_user="me"):
        try:
            ...
            pass
    print(f"[PURGE] Deleted {deleted} messages.")
...
# ============================================================
# NEW COMMAND: .fish (Auto Fishing Workflow)
# ============================================================
...
async def run_fish_workflow(client, chat_id):
    try:
        ...
        # کلیک روی دکمه «بندازش تو یخچال»
        async for message in client.iter_messages(chat_id, limit=3):
            if message.buttons:
                for row in message.buttons:
                    for button in row:
                        if "بندازش تو یخچال" in getattr(button, "text", ""):
                            await button.click()
                            break
                    else: continue
                    break
        ...
        # ۳. ارسال دستور «یخچال میویی»
        await client.send_message(chat_id, "یخچال میویی")
        await asyncio.sleep(4)

        # ۴. خواندن پیام یخچال و کلیک روی ماهی «خام»
        async for message in client.iter_messages(chat_id, limit=3):
            if message.text and message.buttons:
                ...
                for row in message.buttons:
                    for button in row:
                        ...
                        for line in lines:
                            if btn_text in line and "خام" in line:
                                await button.click()
                                target_button_text = btn_text
                                break
                        if target_button_text: break
                    if target_button_text: break
                break

        # ۵. کلیک روی دکمه «بپوخش»
        await asyncio.sleep(4)
        async for message in client.iter_messages(chat_id, limit=3):
            if message.buttons:
                for row in message.buttons:
                    for button in row:
                        if "بپوخش" in getattr(button, "text", ""):
                            await button.click()
                            break
                    else: continue
                    break
        ...
        # ۶. کلیک روی دکمه تیک/تایید نهایی
        await asyncio.sleep(4)
        async for message in client.iter_messages(chat_id, limit=3):
            if message.buttons:
                for row in message.buttons:
                    for button in row:
                        btn_txt = getattr(button, "text", "")
                        if any(x in btn_txt for x in ["🛠", "✅", "➕", "تیک"]) or len(btn_txt.strip()) == 0:
                            await button.click()
                            break
                    else: continue
                    break
...
async def start_fish_loop(event):
    ...
    chat_id = event.chat_id
    await event.edit("🎣 اتوماسیون ماهی فعال شد (هر ۳۱ دقیقه).")
...
async def bot_status_report(event):
    ...
    if cat_chats:
        report.append(f"🐱 **چت‌های فعال .cat:** {len(cat_chats)} چت")
    else:
        report.append("🐱 **حالت .cat:** غیرفعال")

    global fish_task_running
    if fish_task_running and not fish_task_running.done():
        report.append("🎣 **وضعیت .fish:** فعال (هر ۳۱ دقیقه)")
    else:
        report.append("🎣 **وضعیت .fish:** غیرفعال")

    if reply_rules:
        total_rules = sum(len(rules) for rules in reply_rules.values())
        report.append(f"🤖 **ریپلای خودکار:** در {len(reply_rules)} چت ({total_rules} قانون)")
    else:
        report.append("🤖 **ریپلای خودکار:** غیرفعال")

    await event.edit("\n".join(report))


# ============================================================
# OTHER COMMANDS (.ping, .whoami)
# ============================================================
